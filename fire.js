...
function start(){
  ...
  renderFire()
  createFireSource()
  setInterval(calculateFirePropagation, propagationTime)
}
...
function updateFireIntensityPerPixel(currentPixelIndex){
  ...
  const belowPixelIndexIntensity = firePixelArray[belowPixelIndex]
  const newFireIntensity = (belowPixelIndexIntensity - decay < 0) ? 0 : belowPixelIndexIntensity - decay
  ...
  for(let row=0; row < fireHeight; row++){
    ...
    for(let column = 0; column < fireWidth; column++){
      ...
      if(debug === true){
        ...
        html += '<td>'
        html += `<div class="displayIndex">${pixelIndex}</div>`
        html += fireIntensity
        html += '</td>'
